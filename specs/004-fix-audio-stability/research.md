...
| S7 | (não relatado; achado na validação manual V1 com hardware real) Reabrir o app logo após fechar falha com `AUDCLNT_E_UNSUPPORTED_FORMAT` (0x88890008) em ~18 de 20 aberturas consecutivas do AIR 192|4 | Encontrado (V1, 2026-09-03) | Pré-existente à feature 004 (`CreateAndStartCapture`/fallback IEEE float idênticos desde a feature 003 — não é regressão desta feature). **Tentativa 1 (retry com 150ms, testada ao vivo): sem efeito — 18/20 de novo, mesma taxa.** Isso descartou a hipótese inicial de "driver ainda não liberou a sessão" como causa suficiente e apontou para revisão de código: em `TryStartCapture`, quando a tentativa de fallback (IEEE float) TAMBÉM lançava `AUDCLNT_E_UNSUPPORTED_FORMAT`, o `WasapiCapture` dessa segunda tentativa nunca era desinscrito/`Dispose()`d antes de propagar a exceção — um `AudioClient` COM vivo ficava preso ao dispositivo, sabotando toda tentativa seguinte (inclusive as 3 repetições do retry). O mesmo padrão existia em `CreateAndInitOutput` (último ramo, sem try/catch nenhum). Pré-existente à 004, nunca acionado antes porque nada testava reabertura repetida em sequência | (a) Repetição limitada (3 tentativas, 150ms) em `CreateAndStartCapture`/`CreateAndInitOutput` — mantida como rede de segurança para o caso de timing real; (b) `Dispose()` garantido de toda captura/saída de fallback antes de repropagar a exceção, em ambas as funções — a correção que provavelmente resolve o problema de fato | Requer hardware real (mesma classe de `RealHardwarePerformanceBudgetTests`); regressão coberta por re-execução do V1 (20 aberturas) após o fix — **resultado pendente de confirmação com o usuário** |

Severidade (FR-018): S1/S2/S4 **alta** (bloqueiam uso); S7 **alta** (bloqueava 90% das aberturas na
validação manual V1, achado pré-existente à feature 004); S3 **média** (coerência); S5/S6 **média**
