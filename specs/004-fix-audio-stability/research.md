...
| S7 | (não relatado; achado na validação manual V1 com hardware real) Reabrir o app logo após fechar falha com `AUDCLNT_E_UNSUPPORTED_FORMAT` (0x88890008) em ~18 de 20 aberturas consecutivas do AIR 192|4 | Encontrado (V1, 2026-09-03) | Pré-existente à feature 004 (`CreateAndStartCapture`/fallback IEEE float idênticos desde a feature 003 — não é regressão desta feature). Hipótese: o driver do AIR 192|4 não libera a sessão de áudio da instância anterior a tempo do `Start()` seguinte, mesmo com `AudioEngine.Dispose()`→`Stop()` síncrono no `OnExit`; o fallback de formato existente resolve o caso "struct WAVEFORMATEXTENSIBLE vs. plana", não o caso "sessão anterior ainda presa" | Repetição limitada (até 3 tentativas, 150ms entre elas) em `CreateAndStartCapture` quando ambas as variações de formato falham com o mesmo HRESULT — dá tempo do driver liberar a sessão sem virar polling nem laço infinito | Requer hardware real (mesma classe de `RealHardwarePerformanceBudgetTests`); regressão coberta por re-execução do V1 (20 aberturas) após o fix |
